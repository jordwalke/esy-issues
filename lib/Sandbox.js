    ...
    dependencyTree[name] = {
      ...
      dependencyTree: depSpecList.length > 0
        ? buildDependencyTree(
            dependencyBaseDir,
            depSpecList,
            packageDependencyTrace.concat(packageJson.name)
          )
        : {}
    };
  }
...
function readPackageJson(filename): PackageJson {
  ...
  if (packageJson.esy.buildOutOfSource == null) {
    packageJson.esy.buildOutOfSource = true;
  }
  return packageJson;
}
...
function packageInfoKey(env: Environment, packageInfo: PackageInfo) {
  ...
  if (packageInfo.__key == null) {
    ...
    if (process.env.ESY__TEST) {
      packageInfo.__key = `${normalizedName}-${version || '0.0.0'}`;
    } else {
      packageInfo.__key = `${normalizedName}-${version || '0.0.0'}-${h}`;
    }
  }
  return packageInfo.__key;
}
