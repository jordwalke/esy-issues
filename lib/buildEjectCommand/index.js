...
function buildEjectCommand(sandbox, ...args) {

  let defaultEjectDirectory = path.join(sandbox.packageInfo.rootDirectory, '_esy');
  let [ejectDirectory = defaultEjectDirectory] = args;
  ...
  let sandboxPath = (packageInfo, tree, ...path) => {
  ...
  let buildPath = (packageInfo, ...path) => sandboxPath(packageInfo, '_build', ...path);

  let installPath = (packageInfo, ...path) => sandboxPath(packageInfo, '_install', ...path);

  let installTmpPath = (packageInfo, ...path) => sandboxPath(packageInfo, '_insttmp', ...path);
  ...
  traversePackageDependencyTree(sandbox.packageInfo, packageInfo => {
    ...
    let dependencies = Object.keys(packageInfo.dependencyTree).map(dep => packageTarget('build', dep));
