...
function relativeToSandbox(realFromPath, toPath) {
  ...
  let toPathToUse = pathIsInside(realFromPath, realToPath) ? realToPath : toPath;
...
var validatePackageJsonExportedEnvVar = (envVar, config, inPackageName, envVarConfigPrefix) => {
  ...
  if (config.scopes !== undefined) {
    ret.push(envVar + " has a field 'scopes' (plural). You probably meant 'scope'. " + "The owner of " + inPackageName + " likely made a mistake");
  }
  let scopeObj = getScopes(config);
  if (!scopeObj.global) {
    if (!beginsWithPackagePrefix) {
      if (envVar.toUpperCase().indexOf(envVarConfigPrefix) === 0) {
        ret.push("It looks like " + envVar + " is trying to be configured as a package scoped variable, " + "but it has the wrong capitalization. It should begin with " + envVarConfigPrefix + ". The owner of " + inPackageName + " likely made a mistake");
      } else {
        ret.push("Environment variable " + envVar + " " + "doesn't begin with " + envVarConfigPrefix + " but it is not marked as 'global'. " + "You should either prefix variables with " + envVarConfigPrefix + " or make them global." + "The author of " + inPackageName + " likely made a mistake");
      }
    }
  } else {
    // Else, it's global, but better not be trying to step on another package!
    if (!beginsWithPackagePrefix && envVar.indexOf("__") !== -1) {
      ret.push(envVar + " looks like it's trying to step on another " + "package because it has a double underscore - which is how we express namespaced env vars. " + "The package owner for " + inPackageName + " likely made a mistake");
    }
  }
  return ret;
};

function builtInsPerPackage(sandbox, prefix, packageInfo, installDirectory) {
  let {
  ...
  return {
    ...
    [`${prefix}__root`]: builtIn(esy.buildsInSource ? targetPath(sandbox, packageInfo, '_build') : relativeToSandbox(sandbox.packageInfo.rootDirectory, rootDirectory)),
    ...
    [`${prefix}__install`]: builtIn(installDirectory != null ? installDirectory : targetPath(sandbox, packageInfo, '_install')),
...
function addEnvConfigForPackage({ seenVars, errors, normalizedEnvVars }, realPathSandboxRootOnEjectingHost, packageName, packageJsonFilePath, exportedEnv) {
  var nextSeenVars = {};
  var nextErrors = [];
  ...
  for (var envVar in exportedEnv) {
    ...
    if (seenVars[envVar] && seenVars[envVar].config.exclusive) {
      nextErrors.push((seenVars[envVar].config.__BUILT_IN_DO_NOT_USE_OR_YOU_WILL_BE_PIPd ? 'Built-in variable ' : '') + envVar + " has already been set by " + relativeToSandbox(realPathSandboxRootOnEjectingHost, seenVars[envVar].packageJsonPath) + " " + "which configured it with exclusive:true. That means it wants to be the only one to set it. Yet " + packageName + " is trying to override it.");
    }
    if (seenVars[envVar] && config.exclusive) {
      nextErrors.push(envVar + " has already been set by " + relativeToSandbox(realPathSandboxRootOnEjectingHost, seenVars[envVar].packageJsonPath) + " " + "and " + packageName + " has configured it with exclusive:true. " + "Sometimes you can reduce the likehood of conflicts by marking some packages as buildTimeOnlyDependencies.");
    }
    nextSeenVars[envVar] = {
...
function computeEnvVarsForPackage(sandbox, packageInfo) {
  ...
  let errors = [];
  var autoExportedEnvVarsForPackage = builtInsPerPackage(sandbox, envVarConfigPrefix, packageInfo);

  let {
    seenVars,
    errors: nextErrors,
    normalizedEnvVars
  } = addEnvConfigForPackage({ seenVars: globalSeenVars, errors, normalizedEnvVars: [] }, sandbox.packageInfo.rootDirectory, packageName, rootDirectory, autoExportedEnvVarsForPackage);

  for (var envVar in packageJson.esy.exportedEnv) {
    nextErrors = nextErrors.concat(validatePackageJsonExportedEnvVar(envVar, packageJson.esy.exportedEnv[envVar], packageName, envVarConfigPrefix));
  }
  ...
  } = addEnvConfigForPackage({ seenVars, errors: nextErrors, normalizedEnvVars }, sandbox.packageInfo.rootDirectory, packageName, path.join(rootDirectory, 'package.json'), packageJson.esy.exportedEnv);
  ...
  globalGroups.push({
    ...
    packageJsonPath: relativeToSandbox(sandbox.packageInfo.rootDirectory, path.join(rootDirectory, 'package.json')),
...
function targetPath(sandbox, packageInfo, tree, ...path) {
 ...
 */
function calculateEnvironment(sandbox, packageInfo, options = {}) {
  /**
  ...
  function setUpBuiltinVariables(envConfigState) {
    let sandboxExportedEnvVars = Object.assign({
      'esy__sandbox': {
        val: '$ESY__SANDBOX',
        exclusive: true,
        __BUILT_IN_DO_NOT_USE_OR_YOU_WILL_BE_PIPd: true
    ...
    if (dependencies.length > 0) {
      let depPath = dependencies.map(dep => targetPath(sandbox, dep, '_install', 'bin')).join(':');
      let depManPath = dependencies.map(dep => targetPath(sandbox, dep, '_install', 'man')).join(':');
    ...
    envConfigState = addEnvConfigForPackage(envConfigState, sandbox.packageInfo.rootDirectory, "EsySandBox", curRootPackageJsonOnEjectingHost, mapObject(sandbox.env, env => ({
      val: env,
      exclusive: false,
      __BUILT_IN_DO_NOT_USE_OR_YOU_WILL_BE_PIPd: false
    })));
    envConfigState = addEnvConfigForPackage(envConfigState, sandbox.packageInfo.rootDirectory, "EsySandBox", curRootPackageJsonOnEjectingHost, sandboxExportedEnvVars);
    envConfigState = addEnvConfigForPackage(envConfigState, sandbox.packageInfo.rootDirectory, "EsySandBox", curRootPackageJsonOnEjectingHost, {});
  ...
  try {
    ...
    traversePackageDependencyTree(packageInfo, computeEnvVarsForPackage.bind(null, sandbox));
  } catch (err) {
    if (err.code === 'ENOENT') {
...
function printEnvironment(groups) {
  return groups.map(function (group) {
    let headerLines = ['', '# ' + group.packageJson.name + (group.packageJson.version ? '@' + group.packageJson.version : '') + ' ' + group.packageJsonPath];
    let renderingBuiltInsForGroup = false;
    let errorLines = group.errors.map(err => {
      return '# [ERROR] ' + err;
    });
    let envVarLines = group.envVars.map(envVar => {
