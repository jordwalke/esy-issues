...
import type {PackageDb, PackageJson} from './PackageDb';
...
function calculatePackageEnvironment(
  packageDb: PackageDb,
  packageName: string,
  asCurrent: boolean = false
): Array<EnvironmentVar> {
  let {
    packageJson,
    packageJsonFilePath,
    normalizedName
  ...
  let prefix = asCurrent ? 'cur' : normalizedName
  return [
    {
      name: `${prefix}__name`,
    ...
    },
    {
      ...
      value: packageJson.version || null,
    },
    {
      name: `${prefix}__root`,
...
function calculatePackageBuildEnvironment(
  packageDb: PackageDb,
  packageName: string
): Array<EnvironmentVar> {
  let {
  ...
  let env = [
    ...
    },
    {
      ...
      value: isRootPackage
        ...
        : `$esy__build_tree/node_modules/${packageJson.name}/findlib.conf`,
    },
  ];

  env = env.concat(
    // dynamic env vars: cur__<variable_name>
    calculatePackageEnvironment(packageDb, packageName, true),
    // per package env vars: <packagename>__<variable_name>
    calculatePackageEnvironment(packageDb, packageName, false)
  );

  let dependencies = collectDependencies(packageDb, packageName);
  if (dependencies.length > 0) {
    for (let dep of dependencies) {
      env = env.concat(calculatePackageEnvironment(packageDb, dep, false));
    }
  ...
  let customEnvironment = calculateEnvironment(
    packageDb,
    packageJson.name
  );
  for (let group of customEnvironment) {
    env = env.concat(group.envVars);
  }
...
module.exports = {
  ...
  printEnvironment,
  calculatePackageEnvironment,
  calculatePackageBuildEnvironment
};
