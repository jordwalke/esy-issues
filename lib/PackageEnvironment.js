...
function builtInsPerPackage(
  ...
  let isRootPackage = packageJson.name === sandbox.packageInfo.packageJson.name;
  ...
  function setUpBuiltinVariables(envConfigState: EnvironmentConfigState) {
    ...
    if (dependencies.length > 0) {
      let depPath = dependencies
        .map(dep => targetPath(sandbox, packageInfo.dependencyTree[dep], '_install', 'bin'))
        .join(':');
      let depManPath = dependencies
        .map(dep => targetPath(sandbox, packageInfo.dependencyTree[dep], '_install', 'man'))
        .join(':');
    ...
    envConfigState = addEnvConfigForPackage(
      ...
      mapObject(sandbox.env, env => ({val: env, exclusive: false}))
    );
    envConfigState = addEnvConfigForPackage(
      envConfigState,
      sandbox.packageInfo.rootDirectory,
