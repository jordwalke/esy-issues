...
function buildCommand(packageDb: PackageDb, args: Array<string>) {
  ...
  traversePackageDb(
    ...
    (packageJsonFilePath, packageJson) => {
      ...
      prelude.push({
        type: 'define',
        name: `${normalizeName(packageJson.name)}__FINDLIB_CONF`,
        value: `
  path = "${findlibPath}"
  destdir = "${installDir(packageJson.name, 'lib')}"
      ...
      rules.push({
        ...
        exportEnv: ['ESY__SANDBOX', `${normalizeName(packageJson.name)}__FINDLIB_CONF`],
        command: `
mkdir -p $(@D)
echo "$${normalizeName(packageJson.name)}__FINDLIB_CONF" > $(@);
        `.trim(),
      });
...
function getPkgEnv(packageDb, packageName, asCurrent = false) {
  ...
  let isRootPackage = packageJson.name === packageDb.rootPackageName;
  let prefix = asCurrent ? 'cur' : normalizeName(packageJson.name);
  return [
    {
      name: `${prefix}__name`,
