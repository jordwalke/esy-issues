...
function buildEjectCommand(
  ...
  traversePackageDependencyTree(
    ...
    (packageInfo) => {
      ...
      let dependencies = [
        'esy_store',
        ...Object.keys(packageInfo.dependencyTree).map(dep => `${dep}.build`)
      ];
      ...
      rules.push(makePackageRule({
        ...
        command: outdent`
          DIRNAME=\`dirname "$cur__install"\`; \\
          (cd $DIRNAME && \\
           gtar \\
            -czf $cur__install_key.tar.gz \\
            --transform "s,^\.,$cur__install_key," \\
            -C ${targetPath(sandbox, packageInfo, '_install')} .)
        `
      }));
      ...
      if (packageJson.pjc && packageJson.pjc.build) {
        ...
        rules.push(makePackageRule({
          ...
          ? outdent`
            ...
            ${buildCommand};\\
            env | sort > $cur__target_dir/_esy_env
          `
          : outdent`
            if [ ! -d "$cur__install" ]; then \\
              CACHED_BUILD_STATUS=\`curl -o /dev/null -s -w "%{http_code}" -I ${cachedBuildUrl(sandbox, packageInfo)}\`; \\
              if [ "$CACHED_BUILD_STATUS" == "404" ]; then \\
                $(ESY__PREPARE_CURRENT_INSTALL_TREE)\\
                ${buildCommand} && \\
                env | sort > $cur__target_dir/_esy_env; \\
              else \\
                cd $esy__store/_install && \\
                wget "${cachedBuildUrl(sandbox, packageInfo)}" && \\
